...
const SettingsPage: React.FC<SettingsPageProps> = ({ 
  ...
  const ColorSwatches = ({ value, onChange }: { value: string, onChange: (val: string) => void }) => {
      const options = [
          ...
          <div className="flex items-center gap-3">
              {options.map((opt) => (
                  <button
                    ...
                    className={`w-6 h-6 rounded-full ${opt.color} transition-transform hover:scale-110 ${
                        value === opt.name 
                        ? `ring-2 ring-offset-2 ${isDarkMode ? 'ring-white ring-offset-gray-900' : 'ring-gray-900 ring-offset-white'} scale-110` 
                        : ''
                    }`}
                    title={opt.name}
                  />
    ...
    return (
        <div className="animate-fade-in-up max-w-2xl">
            ...
            <SettingRow label="Accent Color" subtext="Personalize your ChatBharat branding.">
                <ColorSwatches value={settings.general?.accentColor || 'Blue'} onChange={(val) => updateSetting('general', 'accentColor', val)} />
            </SettingRow>
            <Divider />
